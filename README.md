...
- **`KEYS`**: The path to the JSON file storing registered API keys. This has been prefilled with the path to a "guest" version of the file that has a "guest" key already registered. When visiting the `/register` endpoint of the API, a uses will recieve an API key, and it will be added to this file as a registered key.
